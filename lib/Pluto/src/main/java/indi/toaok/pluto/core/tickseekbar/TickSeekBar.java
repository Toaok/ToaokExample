...
//        initTickXAndTextXLocation();
    ...
    private void initTrackLocation() {
        if (mR2L) {
            ...
            if (hasAboveText()) {
                mProgressTrack.top = mPaddingTop + mThumbTouchRadius + mDefaultTickTextsHeight + SizeUtils.dp2px(mContext, 5);
            } else {
    ...
    private void initTickTextsYLocation() {
        if (needDrawText()) {
            ...
            if (hasBelowText()) {//both text below seek bar
                ...
                mTickTextY = mPaddingTop + Math.round(mDefaultTickTextsHeight - mTextPaint.descent()) + SizeUtils.dp2px(mContext, -2);
            }
