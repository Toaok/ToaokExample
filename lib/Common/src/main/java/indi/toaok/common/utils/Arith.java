...
public class Arith { //默认除法运算精度
     ...
     * @return 舍后的结果
     */
    public static Double roundToDouble(double v) {
        BigDecimal b = new BigDecimal(Double.toString(v));
        BigDecimal one = new BigDecimal("1");
        return b.divide(one, 4, BigDecimal.ROUND_DOWN).doubleValue();
    }


    public static double pow(double v1, int v2) {
        BigDecimal b1 = new BigDecimal(Double.toString(v1));
        return b1.pow(v2, new MathContext(10, RoundingMode.HALF_UP)).doubleValue();
    }
};    